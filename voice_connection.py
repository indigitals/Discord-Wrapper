...
class VC:
    ...
    async def voice_update(self): #send the op4 voice state update
        await self.connect.send(self.connect.info.returnVoiceUpdate(self.gid, self.cid))
    ...
    async def _init_connection(self):
        ...
        await asyncio.gather(self.connect.send_heartbeat(), self.connect.send_identify(), msg_task) #initialize connection to main gateway
